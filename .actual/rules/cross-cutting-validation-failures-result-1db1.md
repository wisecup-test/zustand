# Enforce Input Validation and Sanitization in Security-Critical Code Paths: Validation Failures Result
...
- **R-VAL-001** MUST: Validation failures MUST result in explicit error handling with safe defaults or rejection of the input.
...
<enforcement>
Claude Code MUST NOT skip or defer verification. Static analysis scanning for unvalidated input patterns is mandatory in CI pipeline. Code review must verify validation is present or approved exception exists before merge. Violations block CI and require security team notification.
</enforcement>